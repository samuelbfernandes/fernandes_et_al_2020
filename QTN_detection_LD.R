...
total_33 <- tibble(
  ...
  ld = character(lfolder),
  maf = character(lfolder),
  type = character(lfolder),
  h2 = "0.3_0.3",
  mt = numeric(lfolder),
  mtt1 = numeric(lfolder),
  mtt2 = numeric(lfolder),
  mtt1t2 = numeric(lfolder),
  t1 = numeric(lfolder),
  t2 = numeric(lfolder),
  t1_and_t2 = numeric(lfolder),
  af1 = numeric(lfolder),
  af2 = numeric(lfolder),
  ld_QTN = numeric(lfolder),
  dist_QTN = numeric(lfolder))
total_38 <- total_33
total_38$h2 <- "0.3_0.8"
total_88 <- total_33
total_88$h2 <- "0.8_0.8"
for (z in seq_along(folder)){
  ...
  ld_between_qtn <- fread(
    paste0(path, folder[z], "/LD_summary_Additive.txt"),
    header = TRUE,
    sep = "\t",
    data.table = F
  )
  d <- dir()[grepl(".gds", dir())]
  gds <- d[grepl(gsub("_ld.*", "", folder[z]), d)]
  data <- vector("list", n_gwas)
  count <- tibble(
    specie = gsub("_.*", "", folder[z]),
    sample_size = gsub(".*_", "", gsub("_ld.*", "", folder[z])),
    ld = gsub(".*ld_|_MAF.*", "", folder[z]),
    maf = gsub(".*MAF|_dir.*|_indir.*", "", folder[z]),
    type = gsub(".*_", "", folder[z]),
    h2 = character(n_gwas),
    rep = numeric(n_gwas),
    mt = numeric(n_gwas),
    mtt1 = numeric(n_gwas),
    mtt2 = numeric(n_gwas),
    mtt1t2 = numeric(n_gwas),
    t1 = numeric(n_gwas),
    t2 = numeric(n_gwas),
    t1_and_t2 = numeric(n_gwas),
    af1 = numeric(n_gwas),
    af2 = numeric(n_gwas),
    ld_QTN = numeric(n_gwas),
    dist_QTN = numeric(n_gwas)
  )
  count_all <- count
  indirect <- gsub(".*_", "", folder[z]) == "indirect"
  dist <- ifelse(gsub("_.*","",folder[z]) == "ames", dist_threshold_ames, dist_threshold_soy)
  total_33$specie[z] <- gsub("_.*", "", folder[z])
  total_33$sample_size[z] <- gsub(".*_", "", gsub("_ld.*", "", folder[z]))
  total_33$ld[z] <- gsub(".*ld_|_MAF.*", "", folder[z])
  total_33$maf[z] <- gsub(".*MAF|_dir.*|_indir.*", "", folder[z])
  total_33$type[z] <- gsub(".*_", "", folder[z])
  total_38$specie[z] <- total_33$specie[z]
  total_38$sample_size[z] <- total_33$sample_size[z]
  total_38$ld[z] <- total_33$ld[z]
  total_38$maf[z] <- total_33$maf[z]
  total_38$type[z] <- total_33$type[z]
  total_88$specie[z] <- total_33$specie[z]
  total_88$sample_size[z] <- total_33$sample_size[z]
  total_88$ld[z] <- total_33$ld[z]
  total_88$maf[z] <- total_33$maf[z]
  total_88$type[z] <- total_33$type[z]
  ...
  for (n in 1:n_gwas){
    ...
    qtn1 <- qtn[qtn$rep == rep & (qtn$snp_type == "QTN_for_trait_1" | qtn$snp_type == "QTN_upstream"), ]
    qtn1_genotype <-
      read.gdsn(
    ...
    qtn2 <- qtn[qtn$rep == rep & (qtn$snp_type == "QTN_for_trait_2" | qtn$snp_type == "QTN_downstream"), ]
    qtn2_genotype <-
      read.gdsn(
        index.gdsn(genofile, "genotype"),
        start = c(1, which(snps%in%qtn2$snp)),
        count = c(-1, 1)
      )
    ...
    gemma_mt <-
      gemma_mt %>%
      arrange(fdr) %>%
      filter(fdr < 0.1 & chr == qtn1$chr & rs != qtn1$snp & rs != qtn2$snp) %>%
      mutate(
        dist_qtn1 = abs(ps - qtn1$pos),
        dist_qtn2 = abs(ps - qtn2$pos),
        rep = rep,
        h2 = h2,
        model = "MT")
    gemma_t1 <-
    ...
    gemma_t2 <-
      gemma_t2 %>%
      arrange(fdr) %>%
      filter(fdr < 0.1 & chr == qtn2$chr & rs != qtn1$snp & rs != qtn2$snp ) %>%
      mutate(
        dist_qtn1 = abs(ps - qtn1$pos),
        dist_qtn2 = abs(ps - qtn2$pos),
        rep = rep,
    ...
    mt <- gemma_mt %>% filter(dist_qtn1 <= dist | dist_qtn2 <= dist)
    mtt1 <- gemma_mt %>% filter(dist_qtn1 <= dist)
    mtt2 <- gemma_mt %>% filter(dist_qtn2 <= dist)
    t1 <- gemma_t1 %>% filter(dist_qtn1 <= dist)
    t2 <- gemma_t2 %>% filter(dist_qtn2 <= dist)
    
    count$mt[n] <- as.numeric(nrow(mt) > 0)
    count$mtt1[n] <- as.numeric(nrow(mtt1) > 0)
    count$mtt2[n] <- as.numeric(nrow(mtt2) > 0)
    count$mtt1t2[n] <- as.numeric(nrow(mtt1) > 0 & nrow(mtt2) > 0)   
    ...
    count$af2[n] <- qtn2$maf
    count$ld_QTN[n] <- ifelse(ncol(ld_between_qtn) == 5,
                              ld_between_qtn[ld_between_qtn$rep%in%rep, 3],
                              ld_between_qtn[ld_between_qtn$rep%in%rep, 8])
    count$dist_QTN[n] <- abs(qtn1$pos - qtn2$pos)
    
    mt2 <- gemma_mt %>% filter(dist_qtn1 >= dist | dist_qtn2 >= dist)
    mtt12 <- gemma_mt %>% filter(dist_qtn1 >= dist)
    mtt22 <- gemma_mt %>% filter(dist_qtn2 >= dist)
    t12 <- gemma_t1 %>% filter(dist_qtn1 >= dist)
    t22 <- gemma_t2 %>% filter(dist_qtn2 >= dist)
    
    count_all$mt[n] <- as.numeric(nrow(mt2) > 0)
    count_all$mtt1[n] <- as.numeric(nrow(mtt12) > 0)
    count_all$mtt2[n] <- as.numeric(nrow(mtt22) > 0)
    count_all$mtt1t2[n] <- as.numeric(nrow(mtt12) > 0 & nrow(mtt22) > 0)   
    ...
    count_all$af2[n] <- qtn2$maf
    count_all$ld_QTN[n] <- ifelse(ncol(ld_between_qtn) == 5,
                                  ld_between_qtn[ld_between_qtn$rep%in%rep, 3],
                                  ld_between_qtn[ld_between_qtn$rep%in%rep, 8])
    count_all$dist_QTN[n] <- abs(qtn1$pos - qtn2$pos)
  ...
  total_33$mtt1[z] <- count_h33 %>% select(mtt1) %>% sum
  total_33$mtt2[z] <- count_h33 %>% select(mtt2) %>% sum
  total_33$mtt1t2[z] <- count_h33 %>% select(mtt1t2) %>% sum
  ...
  total_33$af2[z] <- count_h33 %>% summarise(af1 = median(af2))
  total_33$ld_QTN[z] <- count_h33 %>% summarise(ld_QTN = median(ld_QTN))
  total_33$dist_QTN[z] <- count_h33 %>% summarise(dist_QTN = median(dist_QTN))
  #----
  total_38$mt[z] <- count_h38 %>% select(mt) %>% sum
  total_38$mtt1[z] <- count_h38 %>% select(mtt1) %>% sum
  total_38$mtt2[z] <- count_h38 %>% select(mtt2) %>% sum
  total_38$mtt1t2[z] <- count_h38 %>% select(mtt1t2) %>% sum
  ...
  total_38$af2[z] <- count_h38 %>% summarise(af1 = median(af2))
  total_38$ld_QTN[z] <- count_h38 %>% summarise(ld_QTN = median(ld_QTN))
  total_38$dist_QTN[z] <- count_h38 %>% summarise(dist_QTN = median(dist_QTN))
  
  total_88$mt[z] <- count_h88 %>% select(mt) %>% sum
  total_88$mtt1[z] <- count_h88 %>% select(mtt1) %>% sum
  total_88$mtt2[z] <- count_h88 %>% select(mtt2) %>% sum
  total_88$mtt1t2[z] <- count_h88 %>% select(mtt1t2) %>% sum
  ...
  total_88$af2[z] <- count_h88 %>% summarise(af1 = median(af2))
  total_88$ld_QTN[z] <- count_h88 %>% summarise(ld_QTN = median(ld_QTN))
  total_88$dist_QTN[z] <- count_h88 %>% summarise(dist_QTN = median(dist_QTN))
  count_total[[z]] <- count 
  
  #----------
  total_all_33$mt[z] <- count_all_h33 %>% select(mt) %>% sum
  total_all_33$mtt1[z] <- count_all_h33 %>% select(mtt1) %>% sum
  total_all_33$mtt2[z] <- count_all_h33 %>% select(mtt2) %>% sum
  total_all_33$mtt1t2[z] <- count_all_h33 %>% select(mtt1t2) %>% sum
  ...
  total_all_33$af2[z] <- count_all_h33 %>% summarise(af1 = median(af2))
  total_all_33$ld_QTN[z] <- count_all_h33 %>% summarise(ld_QTN = median(ld_QTN))
  total_all_33$dist_QTN[z] <- count_all_h33 %>% summarise(dist_QTN = median(dist_QTN))
  #----
  total_all_38$mt[z] <- count_all_h38 %>% select(mt) %>% sum
  total_all_38$mtt1[z] <- count_all_h38 %>% select(mtt1) %>% sum
  total_all_38$mtt2[z] <- count_all_h38 %>% select(mtt2) %>% sum
  total_all_38$mtt1t2[z] <- count_all_h38 %>% select(mtt1t2) %>% sum
  ...
  total_all_38$af2[z] <- count_all_h38 %>% summarise(af1 = median(af2))
  total_all_38$ld_QTN[z] <- count_all_h38 %>% summarise(ld_QTN = median(ld_QTN))
  total_all_38$dist_QTN[z] <- count_all_h38 %>% summarise(dist_QTN = median(dist_QTN))
  
  total_all_88$mt[z] <- count_all_h88 %>% select(mt) %>% sum
  total_all_88$mtt1[z] <- count_all_h88 %>% select(mtt1) %>% sum
  total_all_88$mtt2[z] <- count_all_h88 %>% select(mtt2) %>% sum
  total_all_88$mtt1t2[z] <- count_all_h88 %>% select(mtt1t2) %>% sum
  ...
  total_all_88$af2[z] <- count_all_h88 %>% summarise(af1 = median(af2))
  total_all_88$ld_QTN[z] <- count_all_h88 %>% summarise(ld_QTN = median(ld_QTN))
  total_all_88$dist_QTN[z] <- count_all_h88 %>% summarise(dist_QTN = median(dist_QTN))
  count_total[[z]] <- count 
...
# fwrite(count_total, "total_count_all_LD.txt", sep = "\t", quote = F, row.names = F)
# fwrite(total, "total_count_LD.txt", sep = "\t", quote = F, row.names = F)
# fwrite(total_all, "summary_all_LD.txt", sep = "\t", quote = F, row.names = F)
...
ld <- ld[, c(1:5, 45, 6:44)]
#fwrite(ld, "ld_QTN_LD.txt", sep = "\t", quote = F, row.names = F)