source("./gemma.R")
run_gemma_ST <- function(path = getwd(), pop, n_cores = parallel::detectCores()){
  files <- dir(path)
  ...
  invisible(mclapply(files_fam, function(x) {
    genoname <- gsub(".fam", "", x)
    #to avoid two copies of the same file at the same time
    ...
    gemma(
      ...
      maf = 0.0001,
      r2 = 0.9999999,
      out_name = gsub("Simulated_Data__", "T1", genoname),
      path_out = paste0(path,"gemma" )
    )
    gemma(
      geno = genoname,
      ...
      maf = 0.0001,
      r2 = 0.9999999,
      out_name = gsub("Simulated_Data__", "T2", genoname),
      path_out = paste0(path,"gemma" )
    )
    unlink(c(
