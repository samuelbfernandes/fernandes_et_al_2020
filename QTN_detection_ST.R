...
total_33 <- tibble(
  ...
  af1 = numeric(lfolder),
  af2 = numeric(lfolder),
  chr1 = numeric(lfolder),
  chr2 = numeric(lfolder),
...
for (z in seq_along(folder)){
  ...
  if(gsub("_.*", "", folder[z]) == "soy") {
    qtn$chr <- as.numeric(gsub("Gm","",qtn$chr))
  }
  d <- dir()[grepl(".gds", dir())]
  ...
  for (n in 1:n_gwas){
    ...
    gemma_t1 <- fread(
      paste0(path, folder[z], "/gemma/output/", paste0("T1", gsub("MT", "", gemma_gwas_MT[n]))),
      header = TRUE,
      sep = "\t",
      select = c("chr", "rs", "ps", "p_lrt"),
      data.table = F
    ...
    gemma_t2 <- fread(
      paste0(path, folder[z], "/gemma/output/",  paste0("T2", gsub("MT", "", gemma_gwas_MT[n]))),
      header = TRUE,
      sep = "\t",
      select = c("chr", "rs", "ps", "p_lrt"),
      data.table = F
    ...
    count$ld_QTN[n] <- abs(SNPRelate::snpgdsLDpair(qtn1_genotype, qtn2_genotype, method = "composite"))
    snpgdsClose(genofile)

    gemma_mt <-
      gemma_mt %>%
      arrange(fdr) %>%
      filter(fdr < 0.05 & (chr == qtn1$chr | chr == qtn2$chr) & rs != qtn1$snp & rs != qtn2$snp) %>%
      mutate(
    ...
    gemma_t1 <-
      gemma_t1 %>%
      arrange(fdr) %>%
      filter(fdr < 0.05 & chr == qtn1$chr & rs != qtn1$snp ) %>%
      mutate(
        dist_qtn1 = abs(ps - qtn1$pos),
        dist_qtn2 = abs(ps - qtn2$pos),
        rep = rep,
    ...
    gemma_t2 <-
      gemma_t2 %>%
      arrange(fdr) %>%
      filter(fdr < 0.05 & chr == qtn2$chr & rs != qtn2$snp) %>%
      mutate(
        dist_qtn1 = abs(ps - qtn1$pos),
        dist_qtn2 = abs(ps - qtn2$pos),
        rep = rep,
    ...
    count$chr1[n] <- qtn1$chr
    count$chr2[n] <- qtn2$chr    
    ...
    count_all$chr1[n] <- qtn1$chr
    count_all$chr2[n] <- qtn2$chr    
...
# fwrite(count_total, "total_count_all_ST.txt", sep = "\t", quote = F, row.names = F)
# fwrite(total, "total_count_ST.txt", sep = "\t", quote = F, row.names = F)
...
ld <- ld[, c(1:5, 45, 6:44)]
#fwrite(ld, "ld_QTN_ST.txt", sep = "\t", quote = F, row.names = F)
